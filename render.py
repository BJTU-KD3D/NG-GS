...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, train_test_exp, separate_sh, args):
    if not args.include_mask:
        ...
        for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
            ...
            # Use the original image name instead of an index
            if hasattr(view, 'image_name') and view.image_name:
                # Extract the file name from the path and drop the extension
                filename = os.path.splitext(os.path.basename(view.image_name))[0]
                print(f"Using original filename: {filename} for view {idx}")
            else:
                # Fall back to a numeric index if there is no original file name
                filename = '{0:05d}'.format(idx)

            torchvision.utils.save_image(rendering, os.path.join(render_path, f"{filename}.png"))
            torchvision.utils.save_image(gt, os.path.join(gts_path, f"{filename}.png"))
    else:
        ...
        for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
            ...
            torchvision.utils.save_image(depth, os.path.join(depth_path, f'depth_{filename}.JPG'))
            torchvision.utils.save_image(render_image, os.path.join(image_path, f'{filename}.JPG'))
            torchvision.utils.save_image(mask, os.path.join(render_path, f'{filename}.JPG'))
